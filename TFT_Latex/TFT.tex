...
En este trabajo se emplean modelos basados en las aruitecturas MobileNetV2, EfficientNetB0 y NASNetMobile preentrenados con ImageNet como punto de partida, lo que permite centrar el esfuerzo en la adaptación al problema de enfermedades de tomate mediante fine-tuning y validación rigurosa.
...
\begin{enumerate}[label=\bfseries\headlinecolor\arabic*.]
\item Bloques residuales invertidos (\textit{Inverted Residuals}): Primero se expande el número de canales mediante una convolución 1×1 (capa de expansión). Luego se aplica una convolución \textit{"depthwise"} (convolución separada por canal) para realizar el filtrado espacial. Finalmente, se reduce de nuevo el número de canales a la dimensión deseada mediante otra 1×1 (capa de proyección). Esta estructura permite conservar dimensiones de entrada/salida mientras se reduce el coste computacional.
\item Convoluciones separables en profundidad (\textit{Depthwise Separable Convolutions}): Al igual que su antedecesor, utiliza convoluciones separables, es decir, se descompone una convolución estándar en dos operaciones; primero una convolución \textit{"depthwise"} por cada canal, luego una \textit{"pointwise”} 1×1 que mezcla los canales. Esto reduce considerablemente el número de parámetros y de cálculos, lo que mejora la eficiencia.
...
A continuación en el Cuadro \ref{tab:arquitectura_MobileNetV2} se expone una división funcional de la red para entender qué aprenden las capas:

\begin{table}[ht] 
\centering
\caption{\headlinecolor{\underline{División funcional de MobileNetV2}}} %https://uk.mathworks.com/help/deeplearning/ref/mobilenetv2.html
\label{tab:arquitectura_MobileNetV2}
\begin{tabular}{|p{2.5cm}|p{4cm}|p{8cm}|}
\rowcolor{naranja}
\multicolumn{3}{|c|}{\textbf{División funcional de MobileNetV2}} \\ \hline
\rowcolor{naranja!30}
Zona & Capas & Aprendizaje \\ \hline
Zona temprana & Primeros bloques (aproximadamente hasta la capa 20) & Características muy básicas: bordes, texturas simples, colores, orientación, gradientes, es decir, filtros universales que casi cualquier red visual aprende \\ \hline
Zona intermedia & Bloques medios (aproximadamente de la capa 20 a la 80) & Características más complejas: combinaciones de formas, patrones locales, partes de objetos, texturas específicas de hojas/plantas, etc. Aquí ya se empiezan a adaptar a dominios más específicos. \\ \hline
Zona tardía / cabeza del modelo & Últimos bloques (aproximadamente de la capa 80 hasta la final) & Características de muy alto nivel: semánticas del dominio, relaciones globales en la imagen y adaptación al problema concreto \\ \hline
\end{tabular}
\end{table}
...
A continuación en el Cuadro \ref{tab:arquitectura_EfficientNetB0} se expone una división funcional de la red para entender qué aprenden las capas:

\begin{table}[ht] 
\centering
\caption{\headlinecolor{\underline{División funcional de EfficientNetB0}}} %https://uk.mathworks.com/help/deeplearning/ref/mobilenetv2.html
\label{tab:arquitectura_EfficientNetB0}
\begin{tabular}{|p{2.5cm}|p{4cm}|p{8cm}|}
\rowcolor{naranja}
\multicolumn{3}{|c|}{\textbf{División funcional de EfficientNetB0}} \\ \hline
\rowcolor{naranja!30}
Zona & Capas & Aprendizaje \\ \hline
Zona temprana & Desde la entrada (\textit{stem}) hasta los primeros bloques \textit{MBConv} (aproximadamente hasta la capa 20) & Características muy básicas: bordes, texturas simples, colores, orientación, gradientes, es decir, filtros universales que casi cualquier red visual aprende \\ \hline
Zona intermedia & Bloques medios de \textit{MBConv} con mayor expansión y mayor profundidad (aproximadamente de la capa 20 a la 80) & Características más complejas: combinaciones de formas, patrones locales, partes de objetos, texturas específicas de hojas/plantas, etc. Aquí ya se empiezan a adaptar a dominios más específicos. \\ \hline
Zona tardía / cabeza del modelo & Últimos bloques (aproximadamente de la capa 80 hasta la final) & Características de muy alto nivel: semánticas del dominio, relaciones globales en la imagen y adaptación al problema concreto \\ \hline
\end{tabular}
\end{table}
...
\textit{NASNetMobile} es una arquitectura de CNN . Sus características claves son las siguientes \parencite{MobilenetV2Architecture}:
...
A continuación en el Cuadro \ref{tab:arquitectura_NASNetMobile} se expone una división funcional de la red para entender qué aprenden las capas:

\begin{table}[ht] 
\centering
\caption{\headlinecolor{\underline{División funcional de NASNetMobile}}} %https://uk.mathworks.com/help/deeplearning/ref/mobilenetv2.html
\label{tab:arquitectura_NASNetMobile}
\begin{tabular}{|p{2.5cm}|p{4cm}|p{8cm}|}
\rowcolor{naranja}
\multicolumn{3}{|c|}{\textbf{División funcional de NASNetMobile}} \\ \hline
\rowcolor{naranja!30}
Zona & Capas & Aprendizaje \\ \hline
Zona temprana & Primeros bloques (aproximadamente hasta la capa 20) & Características muy básicas: bordes, texturas simples, colores, orientación, gradientes, es decir, filtros universales que casi cualquier red visual aprende \\ \hline
Zona intermedia & Bloques medios (aproximadamente de la capa 20 a la 80) & Características más complejas: combinaciones de formas, patrones locales, partes de objetos, texturas específicas de hojas/plantas, etc. Aquí ya se empiezan a adaptar a dominios más específicos. \\ \hline
...
\section{Modelo \textit{MobileNetV2} 1}\label{sec:sec5.1}
...
Para entender las métricas de la Figura \ref{fig:metrics_summary_mobilenetv2_1_1} se va a hacer una breve explicación de cada una de ellas:
...
Para extraer resultados de la Figura \ref{fig:confusion_matrix_mobilenetv2_1_1} se va a explicar qué es una matriz de confusión.  Una matriz de confusión es una herramienta que permite la visualización del desempeño de un modelo de clasificación. Cada columna representa el número de predicciones de cada clase, mientras que cada fila representa a las instancias en la clase real.
...
En cuanto a los resultados de la Figura \ref{fig:confusion_matrix_mobilenetv2_1_2} son bastantes similares a los de la subsección \ref{sec:sec5.1.1}
...
En general se obtiene un buen rendimiento, existiendo un buen balance entre precisión y sensibilidad en practicamente todas las clases. Todos los valores de ambas métricas se encuentran entre 0.7 y 0.99, demostrando que no existen demasiadas diferencias entre las clases, afirmando la robustez del modelo.

En cuanto a los resultados de la Figura \ref{fig:confusion_matrix_mobilenetv2_1_3} :

\begin{bulletlist}
\item Las clases \texttt{Tomato\_Yellow\_Leaf\_Curl\_Virus}, \texttt{Tomato\_mosaic\_virus} y \texttt{Spidermite\_Two\_spotted\_spider\_mite} son clasificadas casi a la perfección. Cuando nuestro modelo predice esas clases tiene una tas de acierto mayor al 90\%.
\item Las clases \texttt{Target\_Spot}, \texttt{Early\_blight} y \texttt{Septoria\_leaf\_spot} son las más conflictivas, ya que el modelo tiende a clasificar otras imágenes como ellas.
\end{bulletlist}

Este modelo tiene un comportamiento sólido y equilibrado, generaliza bien y consigue que incluso las clases con menos muestras tengan buenos niveles de precisión y sensibilidad. Para seguir refinando este modelo se descongelarán algunas capas del modelo base (fine tuning) y se reducirá el learning rate, estos cambos se aplican en el modelo de la Subsección \ref{sec:sec4.4.2}. 



\section{Modelo \textit{MobileNetV2} 2}\label{sec:sec5.2}
...
En general se obtiene un buen rendimiento, existiendo un buen balance entre precisión y sensibilidad en practicamente todas las clases. Todos los valores de ambas métricas se encuentran entre 0.8 y 0.99, demostrando que no existen demasiadas diferencias entre las clases, afirmando la robustez del modelo.

En cuanto a los resultados de la matriz de confusión de la Figura \ref{fig:confusion_matrix_mobilenetv2_2} :

\begin{bulletlist}
\item Las clases \texttt{Tomato\_Yellow\_Leaf\_Curl\_Virus}, \texttt{Tomato\_mosaic\_virus} y \texttt{healthy} son clasificadas casi a la perfección. Cuando nuestro modelo predice esas clases tiene una tas de acierto mayor al 95\%.
\item Las clases \texttt{Target\_Spot} y \texttt{Early\_blight} son las más conflictivas, ya que el modelo tiende a clasificar otras imágenes como ellas.
\end{bulletlist}

Este modelo tiene un comportamiento sólido y equilibrado, generaliza bien y consigue que incluso las clases con menos muestras tengan buenos niveles de precisión y sensibilidad. El modelo está aprovechando las capas de alto nivel para aprender los patrones específicos de tus hojas de tomate, sin destruir la base entrenada en ImageNet.

Para seguir refinando este modelo se descongelarán más capas del modelo base y se reducirá el learning rate, estos cambos se aplican en el modelo de la Subsección \ref{sec:sec4.4.3}. 


\section{Modelo \textit{MobileNetV2} 3}\label{sec:sec5.3}
...
En general se obtiene un buen rendimiento, existiendo un buen balance entre precisión y sensibilidad en practicamente todas las clases. Todos los valores de ambas métricas se encuentran entre 0.7 y 0.99, demostrando que no existen demasiadas diferencias entre las clases, afirmando la robustez del modelo. Comparandolo con el anterior modelo la única clase que ha mejorado es \texttt{Early\_blight} (mejora sensibilidad de 0.835 a 0.9), pero el resto de clases se mantienen en valores similares o peores.

En cuanto a los resultados de la matriz de confusión de la Figura \ref{fig:confusion_matrix_mobilenetv2_3} :

\begin{bulletlist}
\item Las clases \texttt{Tomato\_Yellow\_Leaf\_Curl\_Virus}, \texttt{Tomato\_mosaic\_virus} y \texttt{healthy} son clasificadas casi a la perfección. Cuando nuestro modelo predice esas clases tiene una tas de acierto mayor al 95\%.
\item Las clases \texttt{Target\_Spot} y \texttt{Septoria\_Leaf\_spot} son las más conflictivas, ya que el modelo tiende a clasificar otras imágenes como ellas.
\end{bulletlist}
...
\section{Modelo \textit{EfficientNetB0} 1}\label{sec:sec5.4}
...
\section{Modelo \textit{EfficientNetB0} 2}\label{sec:sec5.5}
...
\section{Modelo \textit{EfficientNetB0} 3}\label{sec:sec5.6}
...
\section{Modelo \textit{NASNetMobile} 1}\label{sec:sec5.7}
...
\section{Modelo \textit{NASNetMobile} 2}\label{sec:sec5.8}
...
En cuanto a los resultados de la matriz de confusión de la Figura \ref{fig:confusion_matrix_nasnetmobile_2} :

\begin{bulletlist}
\item Las clases \texttt{Tomato\_Yellow\_Leaf\_Curl\_Virus}, \texttt{Tomato\_mosaic\_virus} y \texttt{healthy} son clasificadas casi a la perfección. Cuando nuestro modelo predice esas clases tiene una tas de acierto mayor al 95\%.
\item Las clases \texttt{Septoria\_Leaf\_spot}, \texttt{Early\_blight} y \texttt{Late\_blight} y son las más conflictivas, ya que el modelo tiende a clasificar otras imágenes como ellas.
\end{bulletlist}

Este modelo tiene un comportamiento sólido y equilibrado, generaliza bien y consigue que incluso las clases con menos muestras tengan buenos niveles de precisión y sensibilidad.

Descongelar desde la capa 60 hacia adelante ha sido una excelente elección. Se ha permitido que los bloques finales aprendan patrones específicos de las hojas y texturas de las enfermedades y el learning rate bajo fue clave para no destruir los pesos de ImageNet.


\section{Comparativa de modelos}\label{sec:sec5.9}

En esta sección se van a comparar los mejores modelos que se han hallado en este estudio, es decir, se van a comparar los modelos de las subsecciones \ref{sec:sec4.4.2}, \ref{sec:sec4.4.3} y \ref{sec:sec4.4.8}, a par tir de los resultados obtenidos en las secciones \ref{sec:sec5.2}, \ref{sec:sec5.3} y \ref{sec:sec5.8} respectivamente.
...
NASNetMobile & 3& 60 capas & \texttt{5e-6} & 15 \\ \hline
\end{tabular}
\end{table}
...
\begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{1.15cm}|Y|Y|>{\centering\arraybackslash}p{2cm}|>{\centering\arraybackslash}p{2cm}|Y|Y|} \hline
\rowcolor{naranja}
\multicolumn{7}{|c|}{\textbf{Resumen de métricas}} \\ \hline
\rowcolor{naranja!30}
Modelo & Precisión macro & Precisión weighted & Sensibilidad macro & Sensibilidad weighted & F1-Score macro & F1-Score weighted \\ \hline
1 & 0.931 & 0.931 & 0.931 & 0.931 & 0.931 & 0.931 \\ \hline
2 & 0.921 & 0.921 & 0.921 & 0.921 & 0.921 & 0.921 \\ \hline
3 & 0.924 & 0.924 & 0.924 & 0.924 & 0.924 & 0.924 \\ \hline
\end{tabularx}
\end{table}

Los tres modelos presentan rendimientos muy próximos, con precisiones superiores al 92\%, lo que indica una buena capacidad de generalización en la clasificación de las once clases de enfermedades.

El modelo MobileNetV2 con fine-tuning de 100 capas y learning rate de 1e-5 obtiene las mejores métricas globales, alcanzando un accuracy del 93.9\% y un f1-score macro de 0.939. Este modelo destaca por su rendimiento equilibrado en la mayoría de las clases, con especial solidez en Tomato mosaic virus (f1 = 0.974) y Healthy (f1 = 0.968).

En comparación, el modelo MobileNetV2 (fine-tuning 80) muestra un rendimiento ligeramente inferior (accuracy 92.4\%), aunque mantiene una buena estabilidad entre clases. Las diferencias más notables respecto al modelo anterior se observan en Septoria leaf spot y Spider mites, donde presenta valores de recall algo más bajos.

Por su parte, NASNetMobile (fine-tuning 60) alcanza un rendimiento global similar (accuracy 92.0\%), con un comportamiento especialmente bueno en las clases Spider mites (f1 = 0.962) y Target Spot (f1 = 0.931). Sin embargo, en otras clases como Bacterial spot o Septoria leaf spot su precisión se reduce notablemente, lo que sugiere una menor estabilidad interclase.

En conjunto, los resultados confirman que las arquitecturas basadas en MobileNetV2 ofrecen un mejor compromiso entre rendimiento y eficiencia computacional en este conjunto de datos.
Las diferencias entre los modelos son moderadas (menos de dos puntos porcentuales en accuracy), pero la configuración con fine-tuning más profundo (100 capas) muestra una ligera ventaja consistente en las métricas promedio y una mayor estabilidad entre clases.

Estos resultados serán considerados en el apartado de Conclusiones para la selección del modelo final.
