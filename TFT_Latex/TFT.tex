...
\item Tizón precoz (\textit{Early blight}): El agente causal principal es el hongo \textit{Alternaria solani} (y en algunos casos \textit{Alternaria tomatophila}) que puede sobrevivir en residuos de plantas enfermas, en el suelo, sobre plantas de la familia de las solanáceas y también transmitirse por semillas o plántulas infectadas \parencite{cornell}. Las condiciones favorables para su desarrollo incluyen: temperaturas cálidas (por encima de 25\textdegree), humedad elevada o periodos de hojas mojadas \parencite{pacificnorthwest}. Los primeros síntomas suelen aparecer en las hojas más viejas, situadas en la base de la planta, aparecen manchas oscuras que a menudo presentan anillos concéntricos. Alrededor de esas manchas la hoja puede ponerse amarilla (clorosis) y luego necrosarse, lo que deriva en caída de hojas o defoliación parcial del vegetal \parencite{eorganic} como puede observarse en la Figura \ref{fig:fig3}.
...
\item Mancha bacteriana (\textit{Bacterial spot}): 
\item Picadura de la araña roja (\textit{Spidermite}):
\item Virus de la hoja amarilla (\textit{Yellow leaf curl virus}): 
\end{itemize}
