...
\usepackage{setspace}
\setstretch{1.3} % interlineado 1.3
...
\subsection{Modelo \textit{MobileNetV2} base 1}\label{sec:sec4.4.1}

Este modelo usa de capa base \textit{MobileNetV2} preentrenada con los pesos de ImageNet. Posteriormente se congelan las capas del modelo base para evitar que los pesos se modifiquen durante el entrenamiento y se use correctamente la técnica de transfer learning. A continuación se añade una capa de GlobalAveragePooling y una capa densa con 11 neuronas y función de activación softmax, ya que nos enfrentamos a una clasificación de 11 clases.
...
En el entrenamiento de este modelo solo se están usando los callbacks HistorySaver y ModelCheckpoint.


\subsection{Modelo \textit{MobileNetV2} base 2}\label{sec:sec4.4.2}

Este modelo es igual que el anterior, pero añade una capa de Dropout para mejorar la generalización después de la capa de GlobalAveragePooling.

Imagen de summary

Igualmente en el entrenamiento de este modelo solo se están usando los callbacks HistorySaver y ModelCheckpoint.

\subsection{Modelo \textit{MobileNetV2} optimizado}\label{sec:sec4.4.3}

Este modelo es igual que el anterior. El cambio reside en el entrenamiento.

Imagen de summary

Este modelo además de los callbacks de HistorySaver y ModelCheckpoint también usa los callbacks EarlyStopping y ReduceLROnPlateau. El más relevante en este caso es ReduceLROnPlateau que permite dismunuir el learning rate a lo largo del entrenamiento con el objetivo de mejorar la generalización.

\subsection{Modelo \textit{EfficientNetB0} base}\label{sec:sec4.4.4}
...
\subsection{Modelo \textit{NASNetMobile} base}\label{sec:sec4.4.5}
...
\section{Modelo \textit{MobileNetV2} base 1}\label{sec:sec5.1}

En esta sección se van a mostrar los resultados del modelo descrito en el aprtado \ref{sec:sec4.4.1}. Para ello, en primer lugar se va a mostrar las métricas recogidas en el fichero generado por el callback HistorySaver en la Figura \ref{fig:fig5}.
...
Explicación de que existe overfitting

No es necesario mostrar más debido a overfitting
...
\begin{figure}[ht]
\begin{center}
\includegraphics[scale=0.5]{./Images/historico_metricas_mobilenetv2_3.png}
\caption{\headlinecolor{\underline{Histórico de métricas del modelo MobileNetV2 optimizado}}}
\label{fig:fig6}
\end{center}
\end{figure}
