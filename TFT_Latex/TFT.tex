...
\begin{table}[ht] 
\centering
\caption{\headlinecolor{\underline{Top 10 países productores de tomates 2022 \parencite{wikipedia2}}}}
...
\caption{\headlinecolor{\underline{Tizón tardío en una planta de tomate} \parencite{purdue}}} 
...
\caption{\headlinecolor{\underline{Tizón precoz en una planta de tomate} \parencite{renaissancefarms}}}
...
\caption{\headlinecolor{\underline{Mancha bacteriana en una planta de tomate} \parencite{growingproduce}}}
...
\caption{\headlinecolor{\underline{\parbox{0.9\linewidth}{Picadura de la araña roja de dos manchas en una planta de tomate}} \parencite{inspiredpencil}}}
...
\caption{\headlinecolor{\underline{Virus de la hoja amarilla en una planta de tomate} \parencite{greenlife}}}
...
\item Sklearn: es una librería que contiene un gran número funciones relacionadas con modelos de \textit{machine learning} y \textit{deep learning}. Se ha usado para extraer las principales métricas de los modelos tras su entrenamiento.
\end{bulletlist}
...
La mayoría de clases tienen entre 2000 y 2400 imágenes, lo cual es aceptable. Sin embargo, existen clases claramente minoritarias, como \texttt{Spidermite\_Two-spotted\_spider\allowbreak-\allowbreak\_mite} y \texttt{Target\_Spot} con menos de 1500 imágenes, o  \texttt{powdery\_mildew} con menos de 1000 imágenes. Esto puede causar que el modelo aprenda mejor las clases con más ejemplos y tienda a confundirse en las minoritarias porque tiene menos exposición a ellas.
...
\begin{figure}[ht]
\begin{center}
\includegraphics[scale=0.7]{./Images/Modelos/summary_mobilenetv2_3.png}
...
A continuación se realiza una división funcional de la red para entender qué aprenden las capas \parencite{MobilenetV2}: 
...
\textit{EfficientNet} es una familia de redes neuronales convolucionales (CNN) cuyo objetivo es lograr un alto rendimiento con menos recursos computacionales en comparación con arquitecturas anteriores. Sus características claves son las siguientes \parencite{EfficientnetArchitecture}:
...
A continuación se realiza una división funcional de la red para entender qué aprenden las capas \parencite{Efficientnet}:
...
A continuación se realiza una división funcional de la red para entender qué aprenden las capas \parencite{NASNetMobile}:

\begin{bulletlist}
\item Zona temprana: Comprende los primeros bloques, aproximadamente hasta la capa 50. En esta etapa, la red aprende características muy básicas, como bordes, texturas simples, colores, orientación y gradientes. Se trata de filtros universales que casi cualquier red visual aprende.
\item Zona intermedia: Incluye los bloques medios, aproximadamente desde la capa 50 hasta la 150. Aquí se capturan características más complejas, como combinaciones de formas, patrones locales, partes de objetos y texturas específicas (por ejemplo, de hojas o plantas). En este punto, la red comienza a adaptarse a dominios más específicos.
\item Zona tardía o cabeza del modelo: corresponde a los últimos bloques, desde la capa 150 hasta la final. Esta zona extrae características de muy alto nivel, relacionadas con la semántica del dominio, las relaciones globales dentro de la imagen y la adaptación al problema concreto que se desea resolver.
\end{bulletlist}
...
En esta sección se van a mostrar los resultados del entrenamiento del modelo descrito en la subsección \ref{sec:sec4.4.1}. A este modelo se han aplicado varios entrenamientos distintos explicados en las subsecciones \ref{sec:sec5.2.1}, \ref{sec:sec5.2.2} y \ref{sec:sec5.2.3}.


\subsection{Entrenamiento 1}\label{sec:sec5.2.1}

Este entrenamiento se ha realizado usando los pesos de clases descritos en el Cuadro \ref{tab:pesos_por_clase_entrenamiento}. Solo se ha sometido a 10 épocas y se han usado los callbacks de \textit{ModelCheckpoint} e \textit{HistorySaver}, ya que el objetivo de este entrenamiento es observar la tendencia inicial para poderlo comparar con el entrenamiento de la sección \ref{sec:sec5.2.2} y poder determinar qué tanto puede influir los pesos en las clases.
...
Se puede decir que el modelo de clasificación multiclase alcanzó una precisión del 81.4\% y un F1-score medio de 0.81, lo que indica un buen equilibrio entre precisión y capacidad de detección en las once clases evaluadas. Las clases \texttt{Tomato\_Yellow\_Leaf\allowbreak-\allowbreak\ \_Curl\_Virus}, \texttt{Tomato\_mosaic\_virus} y \texttt{healthy} destacan por sus altos valores de F1-score (superiores a 0,89), mostrando que el modelo identifica correctamente los patrones más distintivos de estas categorías. Asimismo, el desempeño equilibrado entre las métricas macro y ponderadas (\textit{weighted}) sugiere que no existe un sesgo notable hacia clases con mayor cantidad de muestras.
...
El rendimiento global es prácticamente el mismo que el conseguido con el entrenmaiento de la subsección \ref{sec:sec5.2.1}, por lo que para poder compararlos más detalladamente es necesario realizar un análisis de métricas por clases. Para ello se van a mostrar las métricas del método \texttt{classification\_report} de la librería \textit{Sklearn} en la Figura \ref{fig:metrics_summary_mobilenetv2_1_2} y la matriz de confusión en la Figura \ref{fig:confusion_matrix_mobilenetv2_1_2}.
...
En general se obtiene un buen rendimiento, existiendo un buen balance entre precisión y sensibilidad en practicamente todas las clases. Aún así, se pueden observar diferencias en algunas clases con respecto al modelo resultante de la subsección \ref{sec:sec5.2.1}. En concreto se puede determinar una disminución en la métrica de sensibilidad en en clases menos representadas en el \textit{dataset} como las clases \texttt{powdery\_mildew} (de 0.93 a 0.80), \texttt{Septoria\_leaf\_spot} (de 0.71 a 0.61) y \texttt{Spidermite\_Two-spotted\_\-spider\_mite} (de 0.84 a 0.70). Esto demuestra la función de los pesos en las clases, es decir el balanceo de las mismas.

El modelo obtenido en la subsección \ref{sec:sec5.2.1} equilibra mejor los resultados sin sacrificar precisión general, por lo que es la opción más sólida para un sistema de diagnóstico de enfermedades, donde todas las clases tienen importancia clínica. Por tanto  el modelo que se seguirá mejorando en la suubsección subsección \ref{sec:sec5.2.3} 
...
Sin embargo, algunas clases como \texttt{Target\_Spot}, \texttt{Early\_blight} y \texttt{Septoria\_leaf\allowbreak-\allowbreak\ \_spot} presentan valores más bajos, reflejando cierta confusión con otras enfermedades de síntomas visualmente similares.
...
Sin embargo, algunas clases como \texttt{Septoria\_leaf\_spot}, \texttt{Early\_blight} y \texttt{Late\allowbreak-\allowbreak\ \_blight} presentan valores más bajos, reflejando cierta confusión con otras enfermedades de síntomas visualmente similares.
...
\item Se realizaron once entrenamientos, de los cuales tres modelos destacaron por su rendimiento. La comparativa detallada mostró que MobileNetV2 con \textit{fine tuning} de 100 capas y learning rate de \texttt{1e-5} alcanzó los mejores resultados globales, con una precisión del 93.9\% y un \textit{f1-score} macro de 0.939, superando ligeramente al resto de configuraciones y mostrando una mayor estabilidad interclase.
\end{enumerate}
...
Como líneas futuras de trabajo, se plantea incorporar técnicas \textit{ensembles} que permita combinar las diferentes arquitecturas para lograr mejorar la detección de las clases más conflictivas \parencite{research16}.

\appendix

\chapter{Anexo: Código fuente}\label{cap:anexo1}

El código fuente desarrollado para este Trabajo de Fin de Máster se encuentra disponible en el siguiente repositorio: \href{https://github.com/Pr0t0n9/TFM-tomatoes-diseases}{https://github.com/Pr0t0n9/TFM-tomatoes-diseases}. A continuación se va a explicar la estructura de directorios y ficheros del mismo:

\begin{itemize}
    \item Models:
    \begin{itemize}
        \item \texttt{EfficientNetB0\_1}: Contiene fichero con extensión \texttt{.keras} con el modelo descrito en la subsección \ref{sec:sec4.4.4} y un fichero con extensión \texttt{.json} con las métricas recogidas en el entrenamiento.
        \item \texttt{EfficientNetB0\_2}: Contiene fichero con extensión \texttt{.keras} con el modelo descrito en la subsección \ref{sec:sec4.4.5} y un fichero con extensión \texttt{.json} con las métricas recogidas en el entrenamiento.
        \item \texttt{EfficientNetB0\_3}: Contiene fichero con extensión \texttt{.keras} con el modelo descrito en la subsección \ref{sec:sec4.4.6} y un fichero con extensión \texttt{.json} con las métricas recogidas en el entrenamiento.
        \item \texttt{MobileNetV2\_1\_1}: Contiene fichero con extensión \texttt{.keras} con el modelo descrito en la subsección \ref{sec:sec4.4.1} (entrenamiento 1) y un fichero con extensión \texttt{.json} con las métricas recogidas en el entrenamiento.
        \item \texttt{MobileNetV2\_1\_2}: Contiene fichero con extensión \texttt{.keras} con el modelo descrito en la subsección \ref{sec:sec4.4.1} (entrenamiento 2) y un fichero con extensión \texttt{.json} con las métricas recogidas en el entrenamiento.
        \item \texttt{MobileNetV2\_1\_3}: Contiene fichero con extensión \texttt{.keras} con el modelo descrito en la subsección \ref{sec:sec4.4.1} (entrenamiento 3) y un fichero con extensión \texttt{.json} con las métricas recogidas en el entrenamiento.
        \item \texttt{MobileNetV2\_2}: Contiene fichero con extensión \texttt{.keras} con el modelo descrito en la subsección \ref{sec:sec4.4.2} y un fichero con extensión \texttt{.json} con las métricas recogidas en el entrenamiento.
        \item \texttt{MobileNetV2\_3}: Contiene fichero con extensión \texttt{.keras} con el modelo descrito en la subsección \ref{sec:sec4.4.3} y un fichero con extensión \texttt{.json} con las métricas recogidas en el entrenamiento.
        \item \texttt{NASNetMobile\_1}: Contiene fichero con extensión \texttt{.keras} con el modelo descrito en la subsección \ref{sec:sec4.4.7} y un fichero con extensión \texttt{.json} con las métricas recogidas en el entrenamiento.
        \item \texttt{NASNetMobile\_2}: Contiene fichero con extensión \texttt{.keras} con el modelo descrito en la subsección \ref{sec:sec4.4.8} y un fichero con extensión \texttt{.json} con las métricas recogidas en el entrenamiento.
    \end{itemize}
    \item \texttt{TFT\_latex}: Proyecto \textit{Látex} con el que se ha escrito la memoria
    \item \texttt{TFM.ipynb}: \textit{Notebook} usado en \textit{Google Colab} que descarga los datos del repositorio de kaggle y realiza tareas de carga de datos, análisis de datos, preprocesamiento y creación de modelos junto con sus respectivos entrenmientos y evaluaciones.
\end{itemize}
