...
\item Tizón precoz (\textit{Early blight}): El agente causal principal es el hongo \textit{Alternaria solani} (y en algunos casos \textit{Alternaria tomatophila}) que puede sobrevivir en residuos de plantas enfermas, en el suelo, sobre plantas de la familia de las solanáceas y también transmitirse por semillas o plántulas infectadas \parencite{cornell}. Las condiciones favorables para su desarrollo incluyen: temperaturas cálidas (por encima de 25\textdegree), humedad elevada o periodos de hojas mojadas \parencite{pacificnorthwest1}. Los primeros síntomas suelen aparecer en las hojas más viejas, situadas en la base de la planta, aparecen manchas oscuras que a menudo presentan anillos concéntricos. Alrededor de esas manchas la hoja puede ponerse amarilla (clorosis) y luego necrosarse, lo que deriva en caída de hojas o defoliación parcial del vegetal \parencite{eorganic} como puede observarse en la Figura \ref{fig:fig3}.
...
En segundo lugar, se realiza una muestra aleatoria de una imagen por clase, para visualizar el tipo de imágenes que se van a tratar en la Figura \ref{fig:fig7}.
...
En la Figura \ref{fig:fig7} se puede observar la gran variedad de imágenes que existen, con distintos brillos, distintos fondos o incluso giradas:
...
\subsection{Modelo \textit{NASNetMobile} 1}\label{sec:sec4.4.6}
...
Esta información es la que se muestra en la Figura \ref{fig:fig8}.
...
\subsection{Modelo \textit{NASNetMobile} 2}\label{sec:sec4.4.7}
...
\section{\textit{MobileNetV2}}\label{sec:sec4.5.1}
...
\section{\textit{EfficientNetB0}}\label{sec:sec4.5.2}
...
\section{\textit{NASNetMobile}}\label{sec:sec4.5.3}
